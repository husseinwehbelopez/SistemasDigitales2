...
void DAC_dato(int dato)
{
  ...
  DAC_escribeTxRAM(dato & 0xFFF);	// Escribe en TX_RAM el dato que convertir� el DAC
...
#define ADC_BAUD_RATE 2000000			// Baudios de Tx serie del ADC (M�ximo 2000000)
...
int ADC_leeRxRAM(){
  ...
  // El dato le�do se encuentra entre dato1 y dato2 con el siguiente formato:
  //         ---- ----- ----- ---- ---- ---- ---- ----
  ...
  // Reconstruye el dato le�do a partir de dato1 y dato2
  datoLeido = dato2 >> 3;
  datoLeido += dato1 << 5;
  
  // El ADC no entiende de n�meros negativos, y el n�mero en la entrada es negativo si su bit 12 = 1
  // As� que si el n�mero le�do >= 0x0800(2048), su bit 12 = 1 y por tanto debemos convertir en negativo: 0x1000(4096) - entrada
  if (datoLeido & 0x0800){
...
//
// Descripci�n:
//    Devuelve un dato le�do del ADC
//
// Autor: Javier Guill�n �lvarez
//-----------------------------------------------------
int ADC_dato()
{
  ...
  datoLeido = ADC_leeRxRAM();	// Lee de RX_RAM el resultado de la conversi�n
...
//
// Descripci�n: 
//    Compara datoDAC con el dato le�do del ADC y presenta el
//    resultado en el terminal
//
// NOTA: Para realizar el test es preciso conectar la salida anal�gica del DAC
//       con la entrada anal�gica 1 del ADC.
//       La funci�n tiene en cuenta la diferencia de fondos de escala entre el
